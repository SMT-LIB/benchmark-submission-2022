      ...
      (! (=>
           ...
           (let ((result (value map1 element)))
             (=>
               ...
               (and
                 (= result (to_rep
                             (let ((temp___299 (spark_proof_model map1)))
                               (select temp___299 element))))
...
(define-fun tresultS__ref_tresultS__content__7__projection ((a tresultS__ref)) us_t
  (tresultS__content a))
...
;; tresultS__init_wrapper__ref_tresultS__init_wrapper__content__7__projection
(define-fun tresultS__init_wrapper__ref_tresultS__init_wrapper__content__7__projection 
...
(declare-datatypes ((t210b__ref 0))
  (((t210b__refqtmk (t210b__content integer)))))

;; t210b__ref_t210b__content__projection
(define-fun t210b__ref_t210b__content__projection ((a t210b__ref)) integer
  (t210b__content a))
...
(assert
  (forall ((map1 (Array Int character)))
    (! (= (spark_proof_model map1) (let ((temp___296 map1)) temp___296)) :pattern (
    (spark_proof_model
      map1)) )))

;; dynamic_invariant
(define-fun dynamic_invariant4 ((temp___expr_1282 us_t) (temp___is_init_1278 Bool) (temp___skip_constant_1279 Bool) (temp___do_toplevel_1280 Bool) (temp___do_typ_inv_1281 Bool)) Bool
  (=>
    (not (= temp___skip_constant_1279 true))
    (and
      (and
        (dynamic_property
          1
          (length source)
          (first temp___expr_1282)
          (last temp___expr_1282))
        (= (first temp___expr_1282) 1))
      (= (last temp___expr_1282) (length source)))))

;; default_initial_assumption
(define-fun default_initial_assumption ((temp___expr_1284 us_t) (temp___skip_top_level_1285 Bool)) Bool
  (and
    (= (first temp___expr_1284) 1)
    (= (last temp___expr_1284) (length source))))
...
(assert
  (value__function_guard
    (value mapping (to_rep (select (to_array source) j)))
    mapping
...
;; H
(assert
  (= (value mapping (to_rep (select (to_array source) j))) (to_rep
                                                             (select 
                                                             (spark_proof_model
                                                               mapping) 
                                                             (to_rep
...
;; Ensures
(assert (= (to_rep o) (value mapping (to_rep (select (to_array source) j)))))
  ...
  (forall ((k Int))
    (=>
      ...
      (and
        (forall ((k1 Int))
          (value__function_guard
            (value mapping (to_rep (select (to_array source) k1)))
            mapping
...
(assert
  (value__function_guard
    (value mapping (to_rep (select (to_array source) j2)))
    mapping
...
;; H
(assert
  (= (value mapping (to_rep (select (to_array source) j2))) (to_rep
                                                              (select 
                                                              (spark_proof_model
                                                                mapping) 
                                                              (to_rep
...
;; Ensures
(assert
  (= (to_rep o1) (value mapping (to_rep (select (to_array source) j2)))))
...
(assert
  (value__function_guard
    (value mapping (to_rep (select (to_array source) usf)))
    mapping
...
;; H
(assert
  (= (value mapping (to_rep (select (to_array source) usf))) (to_rep
                                                               (select 
                                                               (spark_proof_model
                                                                 mapping) 
                                                               (to_rep
...
;; Goal def'vc
;; File "a-strfix.ads", line 743, characters 0-0
(assert
  (not
  (= (us_attr__init
       (select (to_array1
