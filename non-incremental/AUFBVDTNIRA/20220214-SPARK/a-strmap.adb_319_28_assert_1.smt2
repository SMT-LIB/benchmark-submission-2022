...
(assert
  (ite (= o true)
    (let ((temp___1228 (first seq2)))
      (and
        (=>
          (<= temp___1228 j)
          (and
            (and (<= (first seq2) temp___1228) (<= temp___1228 (last seq2)))
            (and (<= (first seq2) j) (<= j (last seq2)))))
        (let ((temp___1227 (first seq1)))
          (and
            (=>
              (<= temp___1227 j)
              (and
                (and
                  (<= (first seq1) temp___1227)
                  (<= temp___1227 (last seq1)))
                (and (<= (first seq1) j) (<= j (last seq1)))))
            (exists ((j2 Int))
              (and
                (= (let ((temp___1238 (of_array
                                        (to_array seq2)
                                        (first seq2)
                                        j2)))
                     (let ((temp___1236 (of_array
                                          (to_array seq1)
                                          (first seq1)
                                          j2)))
                       (bool_eq4
                         (to_array temp___1236)
                         (first temp___1236)
                         (last temp___1236)
                         (to_array temp___1238)
                         (first temp___1238)
                         (last temp___1238)))) true)
                (and
                  (= (and (ite (dynamic_invariant j2 true true true true)
                            true
                            false) (ite (and
                                          (and
                                            (<= j2 (last seq1))
                                            (<= j2 (last seq2)))
                                          (= (to_rep
                                               (select (to_array seq1) j2)) 
                                          (to_rep
                                            (select (to_array seq2) j2))))
                                     true
                                     false)) true)
                  (and
                    (= spark__branch1 (ite (= j2 2147483647) true false))
                    (and
                            ...
                            (exists ((o4 Bool))
                              (and
                                (ite (and
                                  ...
                                  (= o4 false))
                                (not (= o4 true))))))))))))))))
    (and (= spark__branch1 spark__branch) (= j1 j))))

;; Ensures
(assert
  (forall ((j2 Int))
    (=>
      (and (<= (first seq1) j2) (<= j2 (last seq1)))
      (forall ((k Int))
        (=>
          (and (<= (first seq1) k) (<= k (last seq1)))
          (=>
            (< j2 k)
            (< (to_rep (select (to_array seq1) j2)) (to_rep
                                                      (select (to_array seq1) k)))))))))

;; Ensures
(assert
  (forall ((j2 Int))
    (=>
      (and (<= (first seq2) j2) (<= j2 (last seq2)))
      (forall ((k Int))
        (=>
          (and (<= (first seq2) k) (<= k (last seq2)))
          (=>
            (< j2 k)
            (< (to_rep (select (to_array seq2) j2)) (to_rep
                                                      (select (to_array seq2) k)))))))))
...
;; H
(assert
  (= spark__branch2 (ite (<= j1 (last seq1))
                      (let ((o3 (last seq2)))
                        (let ((lastqtresultqtunusedqtunused o3))
                          (ite (<= j1 o3) true false)))
                      false)))
...
;; H
(assert (= spark__branch3 (ite (<= j1 (last seq1)) true false)))
...
;; Assert
(assert
  (forall ((temp___1250 Int))
    (=>
      (and (<= (first seq2) temp___1250) (<= temp___1250 (last seq2)))
      (not
        (= (to_rep (select (to_array seq2) temp___1250)) (to_rep
                                                           (select (to_array
                                                                    seq1) 
                                                           j1)))))))
...
(assert
  (not
  (= (to_rep (select map1 (to_rep (select (to_array seq1) j1)))) (to_rep
                                                                   (select 
                                                                   (to_array
                                                                    seq1) 
                                                                   j1)))))
