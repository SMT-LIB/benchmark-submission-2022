...
(assert
  (forall ((j Int))
    (=>
      (and (<= (first source) j) (<= j ind1))
      (not (= (match__ source pattern mapping j) true)))))

;; H
(assert
  (= (and (ite (dynamic_property2 (first source) (- (last source) pl1) ind1)
            true
            false) (ite (and
                          (<= (first source) ind1)
...
(define-fun lpeqrpqtresultqtunusedqtunused2 () Bool
  (ite (= ind1 (- (last source) pl1)) true false))

;; H
(assert (not (= ind1 (- (last source) pl1))))
