...
;; Ensures
(assert (in_range1 (- from (first item))))

;; Ensures
(assert (uint_in_range (- from (first item))))

;; eq'result'unused'unused
(define-fun eqqtresultqtunusedqtunused () Bool
  (ite (= (bvsub to__1 (to_rep2 target__first)) (of_int
                                                  (- from (first item))))
    true
    false))
...
;; H
(assert (= (to_rep2 target__last) o1))
...
(define-fun usfqtunusedqtunused () Bool
  (ite (forall ((j (_ BitVec 64)))
         (=>
           (and (bvule (to_rep2 target__first) j) (bvule j to__1))
           (= (us_attr__init
                (select (to_array2
                          (of_array2
                            target1
...
(assert
  (=>
    (bvule (to_rep2 target__first) to__1)
    (and
      (and
        (bvule (first2
                 (of_array2
...
;; H
(assert
  (= (and (ite (<= (first item) usf1) true false) (ite (<= usf1 from)
                                                    true
                                                    false)) true))
