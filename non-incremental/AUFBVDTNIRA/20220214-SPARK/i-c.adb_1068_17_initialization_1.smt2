...
(assert
  (in_range4
    (length
      (bv2nat64 (to_rep2 target__first))
      (bv2nat64 (to_rep2 target__last)))))
...
(assert
  (= spark__branch (ite (< (length
                             (bv2nat64 (to_rep2 target__first))
                             (bv2nat64 (to_rep2 target__last))) (length1
                                                                  item))
                     true
                     false)))
...
(assert
  (to_c__function_guard
    (to_c (to_rep (select (to_array item) from)))
    (to_rep (select (to_array item) from))))
...
;; H
(assert
  (= (to_c (to_rep (select (to_array item) from))) (to_rep
...
;; Ensures
(assert (= (to_rep3 o) (to_c (to_rep (select (to_array item) from)))))
               ...
               (us_tqtmk2
                 (store (to_array2
                          (of_array2
                            target
                            (to_rep2 target__first)
                            (to_rep2 target__last))) to__1 (char16_t__init_wrapperqtmk
                                                             o
                                                             true))
                 (rt2
                   (of_array2
...
;; LoopInvariant
(assert
  (= (bvsub to__2 (to_rep2 target__first)) (of_int (- from1 (first item)))))
...
(assert
  (let ((temp___2217 (of_array2
                       ...
                       (to_rep2 target__first)
                       to__2)))
    (forall ((temp___2218 (_ BitVec 64)))
      (=>
        (and
          (bvule (first2 temp___2217) temp___2218)
          (bvule temp___2218 (last2 temp___2217)))
        (= (us_attr__init (select (to_array2 temp___2217) temp___2218)) true)))))
...
(assert
  (to_c__function_guard
    (to_c (to_rep (select (to_array item) from2)))
    (to_rep (select (to_array item) from2))))
...
;; H
(assert
  (= (to_c (to_rep (select (to_array item) from2))) (to_rep
                                                      (select (to_array item) 
                                                      from2))))
...
;; Ensures
(assert (= (to_rep3 o1) (to_c (to_rep (select (to_array item) from2)))))
...
(declare-const target3 (Array (_ BitVec 64) char16_t__init_wrapper))
               ...
               (us_tqtmk2
                 (store (to_array2
                          (of_array2
                            target2
                            (to_rep2 target__first)
                            (to_rep2 target__last))) to__3 (char16_t__init_wrapperqtmk
                                                             o1
                                                             true))
                 (rt2
                   (of_array2
...
;; Ensures
(assert (in_range1 (- from2 (first item))))

;; Ensures
(assert (uint_in_range (- from2 (first item))))

;; eq'result'unused'unused
(define-fun eqqtresultqtunusedqtunused () Bool
  (ite (= (bvsub to__3 (to_rep2 target__first)) (of_int
                                                  (- from2 (first item))))
    true
    false))
...
;; H
(assert (= (to_rep2 target__first) o4))

;; _f'unused'unused
(define-fun usfqtunusedqtunused () Bool
  (ite (forall ((j (_ BitVec 64)))
         (=>
           (and (bvule (to_rep2 target__first) j) (bvule j to__3))
           (= (us_attr__init
                (select (to_array2
                          (of_array2
                            target3
      ...
      (and
        (bvule (first2
                 (of_array2
                   target3
                   (to_rep2 target__first)
                   (to_rep2 target__last))) (to_rep2 target__first))
...
(define-fun usfqtunusedqtunused1 () Bool
  (ite (forall ((temp___2212 (_ BitVec 64)))
         (=>
           (and
             (bvule (first2
                      (of_array2
...
;; H
(assert
  (= (and (ite (<= (first item) usf1) true false) (ite (<= usf1 from2)
                                                    true
                                                    false)) true))

;; Assert
(assert (and (<= (first item) usf1) (<= usf1 (last item))))

;; H
(assert
  (to_c__function_guard
    (to_c (to_rep (select (to_array item) usf1)))
    (to_rep (select (to_array item) usf1))))
...
;; H
(assert
  (= (to_c (to_rep (select (to_array item) usf1))) (to_rep
                                                     (select (to_array item) 
                                                     usf1))))

;; Ensures
(assert (in_range1 (- usf1 (first item))))

;; Ensures
(assert (uint_in_range (- usf1 (first item))))
...
(assert
  (and
    ...
    (of_int
      (- usf1 (first item)))))
    (bvule (bvadd (to_rep2 target__first) (of_int (- usf1 (first item)))) 
    (last2
      (of_array2 target3 (to_rep2 target__first) (to_rep2 target__last))))))
