...
;; Ensures
(assert (in_range1 (+ (to_rep1 target__first) (- j 1))))

;; (=)'result'unused'unused
(define-fun lpeqrpqtresultqtunusedqtunused () Bool
  (ite (= to__1 (+ (to_rep1 target__first) (- j 1))) true false))

;; Ensures
(assert (uint_in_range (- j 1)))

;; eq'result'unused'unused
(define-fun eqqtresultqtunusedqtunused () Bool
  (ite (= from3 (bvadd (first1 item) (of_int (- j 1)))) true false))
...
(define-fun usfqtunusedqtunused () Bool
  (ite (forall ((j1 Int))
         (=>
           (and (<= (to_rep1 target__first) j1) (<= j1 to__1))
           (= (us_attr__init
                (select (to_array2
                          (of_array2
                            target1
                            (to_rep1 target__first)
                            (to_rep1 target__last))) j1)) true)))
    true
    false))
