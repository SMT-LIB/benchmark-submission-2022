...
;; H
(assert (= trim_nul true))

;; H
(assert (c_length_ghost__function_guard (c_length_ghost item) item))

;; H
(assert (dynamic_invariant5 (c_length_ghost item) true false true true))

;; H
(assert (bvule (c_length_ghost item) (bvsub (last1 item) (first1 item))))

;; H
(assert
  (= (to_rep3
       (select (to_array1 item) (bvadd (first1 item) (c_length_ghost item)))) 0))
...
(assert
  (forall ((j1 (_ BitVec 64)))
    (=>
      (and
        (bvule (first1 item) j1)
        (bvule j1 (bvadd (first1 item) (c_length_ghost item))))
      (=>
        (not (= j1 (bvadd (first1 item) (c_length_ghost item))))
        (not (= (to_rep3 (select (to_array1 item) j1)) 0))))))

;; Goal def'vc
;; File "i-c.ads", line 452, characters 0-0
(assert
  (not (<= (bv2nat64 (c_length_ghost item)) 2147483647)))
