...
(assert
  (forall ((arg (_ BitVec 128)))
    (! (=>
         ...
         (= (big arg) (bv2nat128 arg))) :pattern ((big arg)) )))
...
(assert (= big_2xxdouble (+ (big #xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF) 1)))
...
;; Assume
(assert (dynamic_invariant2 mask true false true true))
...
(assert
  (and
    (and
      (and
        (and
          (hi__function_guard (hi x) x)
          (= (bvand (hi x) mask) #x0000000000000000))
        (<= shift 64))
      (= mask (bvshl #xFFFFFFFFFFFFFFFF (of_int (- 64 shift)))))
    (= y (bvshl x (of_int1 shift)))))

;; (=)'result'unused'unused
(define-fun lpeqrpqtresultqtunusedqtunused () Bool
  (ite (= shift 0) true false))

(declare-const spark__branch Bool)

;; H
(assert (= spark__branch (ite (= shift 0) true false)))
...
;; Ensures
(assert
  (bvule x (bvsub (let ((temp___400 (- 128 shift)))
                    (ite (< temp___400 340282366920938463463374607431768211456)
                      (bvshl #x00000000000000000000000000000001 (of_int1
                                                                  temp___400))
                      #x00000000000000000000000000000000)) #x00000000000000000000000000000001)))

;; Ensures
(assert
  (bvugt (ite (< (- 128 shift) 340282366920938463463374607431768211456)
           (bvshl #x00000000000000000000000000000001 (of_int1 (- 128 shift)))
           #x00000000000000000000000000000000) #x00000000000000000000000000000000))

;; Assert
(assert
  (bvult x (let ((temp___2993 (- 128 shift)))
             (ite (< temp___2993 340282366920938463463374607431768211456)
               (bvshl #x00000000000000000000000000000001 (of_int1
                                                           temp___2993))
...
(assert
  (and
    (big_2xx__function_guard (big_2xx (- 128 shift)) (- 128 shift))
    (< (big x) (big_2xx (- 128 shift)))))

;; Assert
(assert
  (= y (bvmul (ite (< shift 340282366920938463463374607431768211456)
                (bvshl #x00000000000000000000000000000001 (of_int1 shift))
                #x00000000000000000000000000000000) x)))

;; Assert
(assert
  (and
    (big_2xx__function_guard (big_2xx shift) shift)
    (and
      (big_2xx__function_guard (big_2xx shift) shift)
      (and
        (big_2xx__function_guard (big_2xx (- 128 shift)) (- 128 shift))
        (< (* (big_2xx shift) (big x)) (* (big_2xx shift) (big_2xx
                                                            (- 128 shift))))))))

;; Ensures
(assert (in_range (- 128 shift)))
...
(assert
  (and
    ...
    (and
      ...
      (and
        (big_2xx__function_guard
          (big_2xx (+ shift (- 128 shift)))
          (+ shift (- 128 shift)))
        (= (* (big_2xx shift) (big_2xx (- 128 shift))) (ite (= (+ shift (- 128 
                                                         shift)) 128)
                                                         big_2xxdouble
                                                         (big_2xx
                                                           (+ shift (- 128 
...
;; H
(assert
  (ite (< shift 340282366920938463463374607431768211456)
    (= o (bvshl #x00000000000000000000000000000001 (of_int1 shift)))
    (= o #x00000000000000000000000000000000)))

;; Ensures
(assert (= (* (big o) (big x)) (big y)))

;; Goal def'vc
;; File "s-aridou.adb", line 503, characters 0-0
(assert
  (not
  (=>
    (big_2xx__function_guard (big_2xx shift) shift)
    (= (big y) (* (big_2xx shift) (big x))))))
