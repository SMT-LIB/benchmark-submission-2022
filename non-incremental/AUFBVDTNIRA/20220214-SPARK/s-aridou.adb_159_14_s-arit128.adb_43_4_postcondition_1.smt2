...
;; dynamic_invariant
(define-fun dynamic_invariant ((temp___expr_296 (_ BitVec 64)) (temp___is_init_292 Bool) (temp___skip_constant_293 Bool) (temp___do_toplevel_294 Bool) (temp___do_typ_inv_295 Bool)) Bool
  true)
...
;; dynamic_invariant
(define-fun dynamic_invariant2 ((temp___expr_235 Int) (temp___is_init_231 Bool) (temp___skip_constant_232 Bool) (temp___do_toplevel_233 Bool) (temp___do_typ_inv_234 Bool)) Bool
  (=>
    (or
...
;; H
(assert
  (= (big3 x1 x2 x3) (+ (+ (* (* big_2xxsingle big_2xxsingle) (big
                                                                ((_ zero_extend 64) 
                                                                x1))) (* 
  big_2xxsingle (big ((_ zero_extend 64) x2)))) (big ((_ zero_extend 64) x3)))))

;; Assume
(assert (= x1 xx1))

;; Assume
(assert (dynamic_invariant xx1 true false true true))

;; Assume
(assert (= x2 xx2))

;; Assume
(assert (dynamic_invariant xx2 true false true true))

;; Assume
(assert (= x3 xx3))

;; Assume
(assert (dynamic_invariant xx3 true false true true))
...
;; H
(assert (= spark__branch1 (ite (bvugt y3 x3) true false)))
...
(assert
  (ite (= spark__branch1 true)
    (and
      (and
        (= spark__branch (ite (= x2 #x0000000000000000) true false))
        (ite (= spark__branch true)
          (= x11 (bvsub x1 #x0000000000000001))
          (= x11 x1)))
      (= x21 (bvsub x2 #x0000000000000001)))
    (and (and (= spark__branch spark__branch1) (= x11 x1)) (= x21 x2))))

(declare-const x31 (_ BitVec 64))

;; H
(assert (= x31 (bvsub x3 y3)))
...
;; H
(assert (= spark__branch2 (ite (bvugt y2 x21) true false)))

;; H
(assert
  (ite (= spark__branch2 true)
    (= x12 (bvsub x11 #x0000000000000001))
    (= x12 x11)))

(declare-const x22 (_ BitVec 64))

;; H
(assert (= x22 (bvsub x21 y2)))

(declare-const x13 (_ BitVec 64))

;; H
(assert (= x13 (bvsub x12 y1)))

;; Goal def'vc
;; File "s-aridou.adb", line 156, characters 0-0
(assert
  (not (= (big3 x13 x22 x31) (- (big3 x1 x2 x3) (big3 y1 y2 y3)))))
