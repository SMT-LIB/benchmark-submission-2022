...
;; LoopInvariant
(assert (<= scale1 (- 64 shift1)))

;; LoopInvariant
(assert
  (and
    (hi__function_guard (hi zu) zu)
    (not (= (bvand (hi zu) mask1) #x0000000000000000))))

;; LoopInvariant
(assert (= mask1 (bvshl #xFFFFFFFFFFFFFFFF (of_int (- 64 shift1)))))

;; LoopInvariant
(assert (= zu (bvshl (oabs z) (of_int1 scale1))))

;; LoopInvariant
(assert
  (and
    (big_2xx__function_guard (big_2xx scale1) scale1)
    (= (big1 zu) (* (big1 (oabs z)) (big_2xx scale1)))))

;; LoopInvariant
(assert (and (<= 0 inter1) (<= inter1 (- log_single_size 1))))

;; LoopInvariant
(assert (= shift1 (power 2 (- log_single_size inter1))))
