...
;; H
(assert
  (= (and (ite (<= 0 inter) true false) (ite (<= inter (- log_single_size 1))
                                          true
                                          false)) true))

;; Ensures
(assert (in_range1 (- log_single_size inter)))

;; Ensures
(assert (in_range2 (- log_single_size inter)))

;; Goal def'vc
;; File "s-aridou.adb", line 2333, characters 0-0
(assert
  (not (<= (power 2 (- log_single_size inter)) 2147483647)))
