...
;; Assume
(assert (dynamic_invariant2 mask_prev true false true true))

;; Assume
(assert (dynamic_invariant1 zu_prev true false true true))
...
(assert
  (and
    (and
      (and
        (and (<= shift 32) (= zu (bvshl zu_prev (of_int1 shift))))
        (= mask_prev (bvshl #xFFFFFFFFFFFFFFFF (of_int (- 64 (* 2 shift))))))
      (= mask (bvshl #xFFFFFFFFFFFFFFFF (of_int (- 64 shift)))))
    (and
      (hi__function_guard (hi zu_prev) zu_prev)
      (not
        (= (bvand (bvand (hi zu_prev) mask_prev) (bvnot mask)) #x0000000000000000)))))

;; Goal def'vc
;; File "s-aridou.adb", line 2356, characters 0-0
(assert
  (not
  (=>
    (hi__function_guard (hi zu) zu)
    (not (= (bvand (hi zu) mask) #x0000000000000000)))))
