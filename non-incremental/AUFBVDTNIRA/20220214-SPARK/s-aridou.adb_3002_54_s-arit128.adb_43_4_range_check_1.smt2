...
(assert
  (and
    ...
    (and
      (to_uns__function_guard (uc_of_int y) y)
      (= ru (bvsub (uc_of_int x) (uc_of_int y))))))
...
;; Assume
(assert (= (bvsub (uc_of_int x) (uc_of_int y)) ru))

;; Assume
(assert (dynamic_invariant1 ru true false true true))

;; Assert
(assert (= ru (bvsub (of_int x) (of_int y))))

(declare-const spark__branch1 Bool)

;; H
(assert
  (= spark__branch1 (ite (bvult ru #x80000000000000000000000000000000)
                      true
                      false)))
...
;; H
(assert
  (= spark__branch2 (ite (= ru #x80000000000000000000000000000000)
                      true
                      false)))
...
;; Assert
(assert (= r (- (bv2nat128 (bvneg (bvsub (of_int x) (of_int y)))))))

;; Assert
(assert (= r (- (bv2nat128 (bvadd (bvneg (of_int x)) (of_int y))))))
...
(assert
  (not
  (<= (bv2nat128 (bvsub (of_int y) (of_int x))) 170141183460469231731687303715884105727)))
