...
(declare-fun to_uns__function_guard ((_ BitVec 128)
  Int) Bool)
...
;; user_eq
(declare-fun user_eq (int128
  int128) Bool)

(declare-const dummy int128)
...
;; bool_eq
(define-fun bool_eq2 ((x1 (_ BitVec 128)) (y1 (_ BitVec 128))) Bool
  (ite (= x1 y1) true false))
...
;; bool_eq
(define-fun bool_eq3 ((x1 Int) (y1 Int)) Bool
  (ite (= x1 y1) true false))
...
;; user_eq
(declare-fun user_eq2 (tinteger_128B
  tinteger_128B) Bool)

(declare-const dummy2 tinteger_128B)
    ...
    (and
      ...
      (and
        ...
        (and
          ...
          (and
            (to_int__function_guard
              (uc_to_int (bvadd (uc_of_int x) (uc_of_int y)))
              (bvadd (uc_of_int x) (uc_of_int y)))
            (= r (uc_to_int (bvadd (uc_of_int x) (uc_of_int y))))))))))
...
(assert
  (and
    ...
    (and
      (to_uns__function_guard (uc_of_int y) y)
      (= ru (bvadd (uc_of_int x) (uc_of_int y))))))
...
;; Assume
(assert (= (uc_to_int (bvadd (uc_of_int x) (uc_of_int y))) r))

;; Assume
(assert (and (< x 0) (or (< 0 y) (< r 0))))

;; (=)'result'unused'unused
(define-fun lpeqrpqtresultqtunusedqtunused () Bool
  (ite (= x (- 170141183460469231731687303715884105728)) true false))

(declare-const spark__branch Bool)

;; H
(assert
  (= spark__branch (ite (= x (- 170141183460469231731687303715884105728))
                     true
                     false)))
...
(define-fun lpeqrpqtresultqtunusedqtunused1 () Bool
  (ite (= y (- 170141183460469231731687303715884105728)) true false))

(declare-const spark__branch1 Bool)

;; H
(assert
  (= spark__branch1 (ite (= y (- 170141183460469231731687303715884105728))
                      true
                      false)))
...
;; H
(assert (= spark__branch2 (ite (<= y 0) true false)))
...
;; Assume
(assert (= (bvadd (uc_of_int x) (uc_of_int y)) ru))

;; Assume
(assert (dynamic_invariant1 ru true false true true))

;; Assert
(assert (= ru (bvadd (bvneg (of_int (- x))) (of_int y))))

(declare-const spark__branch3 Bool)

;; H
(assert
  (= spark__branch3 (ite (bvult ru #x80000000000000000000000000000000)
                      true
                      false)))
...
(define-fun eqqtresultqtunusedqtunused () Bool
  (ite (= ru #x80000000000000000000000000000000) true false))

(declare-const spark__branch4 Bool)

;; H
(assert
  (= spark__branch4 (ite (= ru #x80000000000000000000000000000000)
                      true
                      false)))
...
;; Assert
(assert
  (= r (- (bv2nat128 (bvneg (bvadd (bvneg (of_int (- x))) (of_int y)))))))

;; Ensures
(assert (in_range1 (- x)))

;; Ensures
(assert (uint_in_range (- x)))

;; Ensures
(assert (uint_in_range y))

;; Goal def'vc
;; File "s-aridou.adb", line 607, characters 0-0
(assert
  (not
  (<= (bv2nat128 (bvadd (bvneg (of_int y)) (of_int (- x)))) 170141183460469231731687303715884105727)))
