...
;; den_pos__def_axiom
(assert
  (= den_pos (bool_eq (to_int2 (bool_lt y 0)) (to_int2 (bool_lt z 0)))))
    ...
    (and
      (and
        (and
          ...
          (= (big1 qu) (abs1 big_q)))
        (and
          (to_int__function_guard (uc_to_int ru) ru)
          (and
            (to_int__function_guard (uc_to_int (bvneg ru)) (bvneg ru))
            (= r (ite (<= 0 x) (uc_to_int ru) (uc_to_int (bvneg ru)))))))
      (and
        (to_int__function_guard (uc_to_int qu) qu)
        (and
          (to_int__function_guard (uc_to_int (bvneg qu)) (bvneg qu))
          (= q (ite (= (<= 0 x) (= den_pos true))
                 (uc_to_int qu)
                 (uc_to_int (bvneg qu)))))))
    (not
      (and
...
;; H
(assert
  (= spark__branch (ite (= (to_int2 (ite (<= 0 x) true false)) (to_int2
                                                                 den_pos))
                     true
                     false)))
...
(assert
  (ite (= spark__branch true)
    (and (<= 0 quot) (and (<= 0 big_q) (and (<= 0 q) (= (big q) big_q))))
    (and
      (not (= (bool_ge x 0) (bool_ge (* (big y) (big z)) 0)))
      (and
        (<= quot 0)
        (and
          (<= big_q 0)
          (and
            (=> (<= 0 x) (<= 0 r))
            (and (=> (< x 0) (<= r 0)) (= (big r) big_r))))))))

;; Goal def'vc
;; File "s-aridou.adb", line 823, characters 0-0
(assert
  (not (= (big q) big_q)))
