...
;; H
(assert
  (= (and (ite (<= (to_rep1 s__first) usf) true false) (ite (<= usf p)
                                                         true
                                                         false)) true))

;; Assert
(assert (and (<= (first s_init) usf) (<= usf (last s_init))))

;; Goal def'vc
;; File "s-imageu.ads", line 96, characters 0-0
(assert
  (not (<= (first (of_array s3 (to_rep1 s__first) (to_rep1 s__last))) usf)))
