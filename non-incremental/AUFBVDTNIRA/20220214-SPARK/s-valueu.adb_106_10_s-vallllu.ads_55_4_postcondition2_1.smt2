...
(declare-const old_acc (_ BitVec 128))
...
;; Assume
(assert (dynamic_invariant2 digit true false true true))

;; Assume
(assert (dynamic_invariant2 base true false true true))

;; Assume
(assert (dynamic_invariant2 old_acc true false true true))
              ...
              (and
                (and
                  (and
                    ...
                    (and (<= (first str) lst) (<= lst (last str))))
                  (and (<= (first str) p) (<= p lst)))
                (let ((temp___1495 (to_rep (select (to_array str) p))))
                  (or
                    (and (<= 48 temp___1495) (<= temp___1495 57))
                    (or
                      (and (<= 97 temp___1495) (<= temp___1495 102))
                      (and (<= 65 temp___1495) (<= temp___1495 70))))))
              (= digit (hexa_to_unsigned_ghost
                         (to_rep (select (to_array str) p)))))
            (= (only_hexa_ghost str p lst) true))
          (and
            (bvule #x00000000000000000000000000000002 base)
            (bvule base #x00000000000000000000000000000010)))
        (=>
          (and
            (bvult digit base)
            (bvule old_acc (bvudiv #xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF 
            base)))
          (= acc (bvadd (bvmul base old_acc) digit))))
      (ite (or
             (or
               (bvuge digit base)
               (bvugt old_acc (bvudiv #xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF 
               base)))
             (and
               (bvugt old_acc (bvudiv (bvadd (bvsub #xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF 
               base) #x00000000000000000000000000000001) base))
               (bvult acc (bvudiv #xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF 
               base))))
        (= overflow true)
        (= (= overflow true) (= old_overflow true))))
    (=>
      (not (= old_overflow true))
      (= (bool_eq8 scan_val (scan_based_number_ghost str p lst base old_acc)) true))))

(declare-const spark__branch Bool)

;; H
(assert (= spark__branch (ite (bvuge digit base) true false)))
...
(assert
  (or
    ...
    (exists ((spark__branch1 Bool))
      (and
        (= spark__branch1 (ite (bvule old_acc (bvudiv (bvadd (bvsub #xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF 
                            base) #x00000000000000000000000000000001) 
                            base))
                            true
                            false))
        (ite (= spark__branch1 true)
          (not (= (scan_overflows_ghost digit base old_acc) true))
          (exists ((spark__branch2 Bool))
            (and
              (= spark__branch2 (ite (bvugt old_acc (bvudiv #xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF 
                                  base))
                                  true
                                  false))
              (or
                ...
                (=
                  (bvult acc (bvudiv #xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF 
                  base))
                  (= (scan_overflows_ghost digit base old_acc) true))))))))))

;; Goal def'vc
;; File "s-valueu.adb", line 74, characters 0-0
(assert
  (not
  (=>
    (not (= overflow true))
