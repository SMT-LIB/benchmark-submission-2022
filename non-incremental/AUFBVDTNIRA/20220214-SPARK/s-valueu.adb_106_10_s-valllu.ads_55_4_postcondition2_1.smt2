...
(declare-const old_acc (_ BitVec 64))
    ...
    (and
      (and
        (and
          ...
          (and
            (bvule #x0000000000000002 base)
            (bvule base #x0000000000000010)))
        (=>
          (and
            (bvult digit base)
            (bvule old_acc (bvudiv #xFFFFFFFFFFFFFFFF base)))
          (= acc (bvadd (bvmul base old_acc) digit))))
      (ite (or
             (or
               (bvuge digit base)
               (bvugt old_acc (bvudiv #xFFFFFFFFFFFFFFFF base)))
             (and
               (bvugt old_acc (bvudiv (bvadd (bvsub #xFFFFFFFFFFFFFFFF 
               base) #x0000000000000001) base))
               (bvult acc (bvudiv #xFFFFFFFFFFFFFFFF base))))
        (= overflow true)
...
(assert
  (or
    ...
    (exists ((spark__branch1 Bool))
      (and
        (= spark__branch1 (ite (bvule old_acc (bvudiv (bvadd (bvsub #xFFFFFFFFFFFFFFFF 
                            base) #x0000000000000001) base))
                            true
                            false))
        (ite (= spark__branch1 true)
          ...
          (exists ((spark__branch2 Bool))
            (and
              (= spark__branch2 (ite (bvugt old_acc (bvudiv #xFFFFFFFFFFFFFFFF 
                                  base))
                                  true
                                  false))
              (or
