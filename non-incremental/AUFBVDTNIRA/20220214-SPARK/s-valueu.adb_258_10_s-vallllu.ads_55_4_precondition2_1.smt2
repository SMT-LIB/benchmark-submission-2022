...
;; Assert
(assert (and (<= (first str1) from) (<= from (last str1))))

;; (=)'result'unused'unused
(define-fun lpeqrpqtresultqtunusedqtunused () Bool
  (ite (= (to_rep (select (to_array str1) from)) 95) true false))

(declare-const spark__branch1 Bool)

;; H
(assert
  (= spark__branch1 (ite (= (to_rep (select (to_array str1) from)) 95)
                      true
                      false)))
...
(assert
  (dynamic_invariant2
    (hexa_to_unsigned_ghost (to_rep (select (to_array str1) from)))
    true
    false
    true
    true))
...
(assert
  (= (hexa_to_unsigned_ghost (to_rep (select (to_array str1) from))) 
  (ite (and
         (<= 48 (to_rep (select (to_array str1) from)))
         (<= (to_rep (select (to_array str1) from)) 57))
    (bvsub (of_int (to_rep (select (to_array str1) from))) #x00000000000000000000000000000030)
    (ite (and
           (<= 97 (to_rep (select (to_array str1) from)))
           (<= (to_rep (select (to_array str1) from)) 102))
      (bvadd (bvsub (of_int (to_rep (select (to_array str1) from))) #x00000000000000000000000000000061) #x0000000000000000000000000000000A)
      (ite (and
             (<= 65 (to_rep (select (to_array str1) from)))
             (<= (to_rep (select (to_array str1) from)) 70))
        (bvadd (bvsub (of_int (to_rep (select (to_array str1) from))) #x00000000000000000000000000000041) #x0000000000000000000000000000000A)
        (to_rep2 dummy4))))))
...
(assert
  (=
    ...
    (or
      (or
        (bvuge (hexa_to_unsigned_ghost
                 (to_rep (select (to_array str1) from))) base)
...
;; H
(assert
  (= spark__branch2 (scan_overflows_ghost
                      (hexa_to_unsigned_ghost
...
(assert
  (not (= (only_hexa_ghost str1 (+ from 1) to__) true)))
