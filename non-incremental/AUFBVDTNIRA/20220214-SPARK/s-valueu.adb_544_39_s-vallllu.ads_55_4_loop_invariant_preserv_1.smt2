...
;; LoopInvariant
(assert (and (<= p2 p5) (<= p5 last_num_based)))

;; LoopInvariant
(assert
  (and
    (let ((temp___1234 (to_rep1 (select (to_array str) p5))))
      (or
        (and (<= 48 temp___1234) (<= temp___1234 57))
        (or
          (and (<= 97 temp___1234) (<= temp___1234 102))
          (and (<= 65 temp___1234) (<= temp___1234 70)))))
    (= digit3 (hexa_to_unsigned_ghost (to_rep1 (select (to_array str) p5))))))

;; LoopInvariant
(assert (=> (= overflow2 true) (= overflow4 true)))
...
(assert
  (=>
    ...
    (= (bool_eq9
         based_val
         (scan_based_number_ghost str p5 last_num_based base2 uval4)) true)))

;; Goal def'vc
;; File "s-valueu.ads", line 368, characters 0-0
(assert
  (not (= (to_rep ptr__pointer_value5) (+ last_num_init 1))))
