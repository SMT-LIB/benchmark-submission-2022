...
(define-fun lpeqrpqtresultqtunusedqtunused () Bool
  (ite (= (to_rep1 (select (to_array str) p4)) base_char1) true false))

(declare-const spark__branch10 Bool)

;; H
(assert
  (= spark__branch10 (ite (= (to_rep1 (select (to_array str) p4)) base_char1)
                       true
                       false)))
...
(define-fun lpeqrpqtresultqtunusedqtunused1 () Bool
  (ite (= (to_rep1 (select (to_array str) p4)) 95) true false))

(declare-const spark__branch11 Bool)

;; H
(assert
  (= spark__branch11 (ite (= (to_rep1 (select (to_array str) p4)) 95)
                       true
                       false)))
...
;; H
(assert (<= (+ p4 1) last_num_based))

;; H
(assert
  (ite (= (of_int1 1) true)
    (let ((temp___743 (to_rep1 (select (to_array str) (+ p4 1)))))
      (or
        (and (<= 48 temp___743) (<= temp___743 57))
        (or
          (and (<= 97 temp___743) (<= temp___743 102))
          (and (<= 65 temp___743) (<= temp___743 70)))))
    (let ((temp___747 (to_rep1 (select (to_array str) (+ p4 1)))))
...
(assert
  (=>
    ...
    (= (bool_eq9
         based_val
         (scan_based_number_ghost str (+ p4 1) last_num_based base2 uval4)) true)))
...
;; H
(assert (= (to_rep ptr__pointer_value4) (to_rep ptr__pointer_value5)))

;; H
(assert (dynamic_invariant5 p5 true true true true))
...
(assert
  (not (= (only_hexa_ghost str p6 last_num_based) true)))
