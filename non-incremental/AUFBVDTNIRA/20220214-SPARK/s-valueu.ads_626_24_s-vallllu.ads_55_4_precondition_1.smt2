    ...
    (forall ((from1 Int) (to__1 Int))
      (forall ((base1 (_ BitVec 128)) (acc1 (_ BitVec 128)))
        (! (=>
             ...
             (ite (< to__1 from1)
               (let ((temp___overflow_318 (of_int1 0)))
                 (us_repqtmk
               ...
               (ite (= (to_rep (select (to_array str1) from1)) 95)
                 ...
                 (ite (= (scan_overflows_ghost
                           ...
                           base1
                           acc1) true)
                   (let ((temp___overflow_319 (of_int1 1)))
                     (us_repqtmk
...
(assert
  ...
  (or
    (and
      (<= 48 (to_rep (select (to_array str) from)))
      (<= (to_rep (select (to_array str) from)) 57))
    (or
      (and
        (<= 97 (to_rep (select (to_array str) from)))
        (<= (to_rep (select (to_array str) from)) 102))
      (and
        (<= 65 (to_rep (select (to_array str) from)))
        (<= (to_rep (select (to_array str) from)) 70))))))
